...
name=app-base-test        # Set your build config name here
appport=5000
...
oc new-app image-registry.openshift-image-registry.svc:5000/"$appns"/"$name"

oc create --edit  route edge "route-$name" --service="$name" -n "$appns" --port="$appport"
